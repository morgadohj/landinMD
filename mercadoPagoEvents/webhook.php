...
if (empty($input)) {
    error_log("Webhook: Input vacío recibido");
    http_response_code(400);
    echo "Empty input";
    exit;
}

// Validar headers de seguridad de MercadoPago
$signature = $_SERVER['HTTP_X_SIGNATURE'] ?? '';
$requestId = $_SERVER['HTTP_X_REQUEST_ID'] ?? '';

if (empty($signature)) {
    error_log("Webhook: Firma de seguridad faltante");
    http_response_code(400);
...
// MP puede enviar payloads con distintas formas: { entity, action, id } o { type, action, data: { id } }
$entity = $data['entity'] ?? null;
$type = $data['type'] ?? null;
$action = $data['action'] ?? null;
$id = $data['id'] ?? ($data['data']['id'] ?? null);

// Para notificaciones de suscripciones, aceptar entity o type = 'preapproval'
$topic = ($entity === 'preapproval' || $type === 'preapproval') ? 'preapproval' : null;
