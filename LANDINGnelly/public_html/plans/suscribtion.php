...
if (!$company_id || !$package_type || !$plan_id) {
    log_debug('Parámetros faltantes: company_id=' . ($company_id ?? 'NULL') . ', package_type=' . ($package_type ?? 'NULL') . ', plan_id=' . ($plan_id ?? 'NULL'));
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Faltan parámetros requeridos: company_id, package_type, plan_id']);
    exit;
}

try {
    log_debug('Iniciando generación de enlace para: company_id=' . $company_id . ', package_type=' . $package_type . ', plan_id=' . $plan_id);
